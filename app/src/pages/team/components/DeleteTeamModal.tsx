...
export default function DeleteTeamModal({
  ...
  const matches = confirmText.trim() === team.name;
      ...
      <div className="w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200 dark:border-slate-800">
          <h2 className="text-sm font-medium text-slate-900 dark:text-white flex items-center gap-1.5">
            <AlertTriangle size={15} className="text-red-500" /> Delete Team
          </h2>
        ...
        <div className="p-5 space-y-3">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            This permanently deletes <span className="font-medium text-slate-800 dark:text-slate-200">{team.name}</span>{" "}
            and every project inside it — requirements, test plans, test cases, test cycles, and work items, for
            every member. This cannot be undone.
          </p>
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">
              Type <span className="font-medium text-slate-700 dark:text-slate-300">{team.name}</span> to confirm
            </label>
            <input
              autoFocus
              value={confirmText}
